...
type Message.t += Render of string | Enter_alt_screen | Exit_alt_screen | Tick
...
let rec run t =
  let () =
    match receive () with
    | Tick -> flush t
    | Render output -> handle_render t output
    | Enter_alt_screen -> handle_enter_alt_screen t
    | Exit_alt_screen -> handle_exit_alt_screen t
    | _ -> ()
  in
  run t

and flush t =
  if is_empty t || same_as_last_flush t then (
    ())
  else
...
let spawn_link ~fps = spawn_link @@ init ~fps

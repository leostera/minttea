...
let keyword fmt = Spices.(default |> faint true |> fg dark_gray |> bold true |> build) fmt
...
let view model =
  if model.counter = -1 then keyword "goodbye!"
    ...
    let first_6 = model.keys |> List.to_seq |> Seq.take 6 |> List.of_seq |> String.concat "" in
    let rest = model.keys |> List.to_seq |> Seq.drop 6 |> List.of_seq |> String.concat "" in
